...
function calculateAvg(row) {
  const test = parseFloat(row.querySelector(".test-input").textContent) || 0;
  const exam = parseFloat(row.querySelector(".exam-input").textContent) || 0;
...
function deleteItem(type, index) {
  if (confirm("Delete this record?")) { db[type].splice(index, 1); saveAndRefresh(); }
}
